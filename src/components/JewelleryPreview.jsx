...
export default function JewelleryPreview({
  ...
  }[productType] || { cx: 260, cy: 180, rx: 130, ry: 105, inner: 0.72, outer: 1.32, label: 'bracelet' };

  const engravingText = engraving.trim().toUpperCase();
            ...
            <g transform={`rotate(${rotation} 260 185)`} filter="url(#shadow)">
              ...
              {engravingText ? (
                <text
                  ...
                  fontFamily="Georgia, serif"
                  letterSpacing="0.8"
                  fill="rgba(32,24,16,0.72)"
                >
                  {engravingText.slice(0, 8)}
                </text>
              ) : null}
              {placements.map((p) => {
                const pos = getPlacementXY(p.angle);
